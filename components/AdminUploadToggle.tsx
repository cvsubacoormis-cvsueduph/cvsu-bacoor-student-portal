...
export default function AdminUploadToggle({
  initialEnabled,
}: AdminUploadToggleProps) {
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    try {
      const success = await setSetting(
        "UPLOAD_GRADES_ENABLED",
        checked.toString(),
      );
      if (success) {
        toast.success(
          `Grade uploading has been ${checked ? "enabled" : "disabled"}.`,
        );
      } else {
