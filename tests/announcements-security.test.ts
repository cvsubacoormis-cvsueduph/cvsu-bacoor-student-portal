...
describe("DELETE /api/announcements — Role guard", () => {
  ...
  it("returns 401 when unauthenticated", async () => {
    setAuthUnauthenticated();
    const req = createMockRequest("DELETE", "http://localhost/api/announcements?id=1");
...
describe("PUT /api/announcements — Role guard", () => {
  ...
  it("returns 401 when unauthenticated", async () => {
    setAuthUnauthenticated();
    const res = await PUT(createMockRequest("PUT", "http://localhost/api/announcements", { ...body, id: 1 }));
    expect(res.status).toBe(401);
  });
  ...
  it("allows admin to update", async () => {
    setAuthAdmin();
    const res = await PUT(createMockRequest("PUT", "http://localhost/api/announcements", { ...body, id: 1, title: "Updated" }));
    expect(res.status).toBe(200);
  });
});
